...
- **Intelligent Tree/Pyramid Layout:** When focus mode is activated, applies a directed BFS algorithm (`compactLayout.ts`) that creates a visually appealing tree/pyramid structure: (1) calculates node levels from START node following edge direction, (2) groups nodes by hierarchy level in horizontal rows, (3) spreads nodes horizontally within each level instead of vertical stacking, (4) uses actual/fallback node dimensions for size-aware spacing (horizontal: 1.2× base gap, vertical: 1.5× base gap between levels), (5) handles orphaned/disconnected nodes by placing them in a final row, (6) saves original positions and restores them when focus mode is turned off, (7) smoothly repositions nodes with animation and auto-fits the view with 350ms delay for layout settling.
