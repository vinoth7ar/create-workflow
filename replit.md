...
- **Focus Mode Button:** Fixed-position button (bottom-right corner) with Maximize2/Minimize2 icons. Toggles hiding all sidebars (left builder sidebar and right node editor) to focus only on the graph canvas. Canvas maintains explicit dimensions (h-screen flex container with overflow-hidden and absolute positioned wrapper) to ensure full viewport coverage without white screen issues.
