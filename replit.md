...
- **Node Editor Sidebar Controls:** Three control buttons on the sidebar header:
  1. **Collapse Button:** ChevronLeft (expanded) / ChevronRight (collapsed) icon toggles **bidirectional collapse** with smooth 300ms transition. Collapses both width (21rem → 3rem) AND height (100% → 3rem). When collapsed and not dragged, positions itself at **top-right corner** (top: 1rem, right: 1rem) as a **horizontal toolbar**. Maintains user-dragged position through collapse/expand cycles. All three buttons remain visible in horizontal layout when collapsed.
...
- **Focus Mode Button:** Fixed-position button (bottom-right corner) with Maximize2/Minimize2 icons. Toggles hiding the left builder sidebar to focus on the graph canvas. The Node Editor Sidebar (right side) remains functional - it appears when nodes are selected, allowing users to edit nodes even in focus mode. When activated, **intelligently rearranges nodes** using a directed BFS algorithm to minimize gaps while respecting node dimensions, then smoothly animates and fits the view. Canvas maintains explicit dimensions (h-screen flex container with overflow-hidden and absolute positioned wrapper) to ensure full viewport coverage without white screen issues.
...
- **Intelligent Horizontal Growth Layout:** When focus mode is activated, applies a parent-anchored horizontal layout algorithm (`compactLayout.ts`) that flows left-to-right: (1) calculates hierarchy levels via directed BFS from START node, (2) positions nodes in columns based on level with fixed horizontal gaps (160px), (3) single child nodes align on same Y-axis as parent, multiple children distribute symmetrically above/below parent's center, (4) collision detection prevents vertical overlap within columns using occupied band tracking, (5) handles orphaned/disconnected nodes by placing them in appropriate columns, (6) saves original positions before layout and restores them when focus mode is turned off, (7) uses actual/fallback node dimensions for size-aware spacing (horizontal: 160px between columns, vertical: 80px between children), (8) smoothly repositions nodes with animation and auto-fits the view with 350ms delay for layout settling.
