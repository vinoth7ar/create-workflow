...
- **Intelligent Compact Layout:** When focus mode is activated, applies a directed BFS algorithm (`compactLayout.ts`) that: (1) calculates node levels from START node following edge direction, (2) groups nodes by hierarchy level in columns, (3) uses actual/fallback node dimensions for size-aware spacing (horizontal: 80px gap, vertical: 60px gap), (4) handles orphaned/disconnected nodes by placing them in a final column, (5) smoothly repositions nodes with animation and auto-fits the view with 150ms delay for layout settling.
