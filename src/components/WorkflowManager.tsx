...
export const transformWorkflowData = (rawData: RawWorkflowData): WorkflowData => {
  ...
  if (workflowData.nodes) {
    ...
    console.log(
      '  - Type:',
      Array.isArray(workflowData.nodes) ? 'Array' : typeof workflowData.nodes
    );
  ...
  if (workflowData.edges) {
    ...
    console.log(
      '  - Type:',
      Array.isArray(workflowData.edges) ? 'Array' : typeof workflowData.edges
    );
  ...
  const transformNodes = (rawNodes: any[]): WorkflowNode[] => {
    ...
    const transformed = rawNodes.map((node, index) => {
      ...
      const type: 'status' | 'event' =
        node.type?.toLowerCase() === 'status' ||
        node.nodeType?.toLowerCase() === 'status' ||
        node.node_type?.toLowerCase() === 'status'
          ? 'status'
          : 'event';
  ...
  const transformEdges = (rawEdges: any[]): WorkflowEdge[] => {
    ...
    const transformed = rawEdges.map((edge, index) => {
      ...
      console.log(
        `    ✅ Transformed: id=${id}, source=${source}, target=${target}, label=${label}`
      );
...
export const analyzeGraphStructure = (workflowData: WorkflowData) => {
  ...
  const startNodes =
    rootNodes.length > 0
      ? rootNodes
      : nodes
          .sort((a, b) => (outgoing.get(b.id)?.length || 0) - (outgoing.get(a.id)?.length || 0))
          .slice(0, 1);
...
export const calculateSmartLayout = (
  ...
  if (isHorizontal) {
    ...
    for (let level = 0; level <= maxLevel; level++) {
      ...
      const totalHeight =
        nodesInLevel.length * stageHeight + (nodesInLevel.length - 1) * nodeSpacing;
...
export const generateSmartEdges = (
  ...
  const generatedEdges = workflowData.edges
    .map((edge, index) => {
      ...
      if (!sourcePos || !targetPos) {
        console.error(
          `❌ Edge ${edge.id} references invalid nodes - source: ${edge.source}, target: ${edge.target}`
        );
        return null;
      }
      ...
      console.log(
        `✅ Processing edge ${edge.id}: ${edge.source} → ${edge.target} (levels: ${sourceLevel} → ${targetLevel})`
      );
      ...
      return {
        ...
        type:
          isSelectedEdge || isConnectedToSelectedNode
            ? 'animated'
            : isBackwardFlow
              ? 'smoothstep'
              : 'bezier',
...
export const WorkflowManager = ({
  ...
  const { processedNodes, processedEdges } = useMemo(() => {
    ...
    try {
      ...
      const reactFlowNodes = generateReactFlowNodes(currentWorkflowData, layout, isHorizontal).map(
        (node) => ({
          ...node,
          data: {
            ...node.data,
            isHighlighted: selectedEdgeId
              ? currentWorkflowData.edges.some(
                  (edge) =>
      ...
      const reactFlowEdges = generateSmartEdges(
        currentWorkflowData,
        layout,
        isHorizontal,
        selectedNodeId,
  ...
  useEffect(() => {
    ...
    if (styledNodes.length > 0) {
      ...
      // Only set initialized if not already initialized to prevent unnecessary updates
      if (!isInitialized) {
    ...
    console.groupEnd();
  }, [styledNodes, styledEdges, setNodes, setEdges, focusMode, isInitialized]);
  ...
  const onNodeClick = useCallback(
    (event: React.MouseEvent, node: Node) => {
      ...
      if (
        newSelectedNodeId &&
        hideWhileSelection &&
        hasMultipleConnections(newSelectedNodeId, processedEdges)
      ) {
        console.log(
          '🎯 Triggering focus mode for node with multiple connections:',
          newSelectedNodeId
        );
  ...
  const onEdgeClick = useCallback(
    (event: React.MouseEvent, edge: Edge) => {
      ...
      if (newSelectedEdgeId && hideWhileSelection) {
        ...
        const focusResult = calculateEdgeFocusMode(
          newSelectedEdgeId,
          processedNodes,
          processedEdges
        );
  ...
  return (
    <div className='w-full h-screen bg-gray-50'>
      ...
      <div className='bg-black text-white px-6 py-4 flex items-center gap-3'>
        ...
        <h1 className='text-lg font-semibold'>{currentWorkflowData.name}</h1>
      </div>
